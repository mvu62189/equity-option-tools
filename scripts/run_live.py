...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run live ingestion pipeline")
...
async def main() -> None:
    ...
    if now_et.strftime("%H:%M") >= config.market_close_freeze_time:
        logging.getLogger(__name__).info(
            "run_live skipped because current time is after market_close_freeze_time=%s",
            config.market_close_freeze_time,
        )
        return
    lock = None
    if config.stream_lock_enforced and not args.allow_shared:
        lock = StreamRunLock(symbol=args.ticker, mode="run_live")
        lock.acquire()
    rate_curve = (
